...
import { createComplaintCategory, updateComplaintCategory } from '../../api/complaintCategories.js'
...
export default function ComplaintCategoryForm({ category, onClose, onSaved }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (isEditMode) {
        await updateComplaintCategory(category.complaintCategoryId, payload)
      } else {
        await createComplaintCategory(payload)
      }
      onSaved()
    } catch (err) {
      setStatus('idle')
  ...
  return (
    <SidePanel
      ...
      subtitle={isEditMode ? 'Update this complaint category.' : 'Add a new complaint category.'}
      onClose={onClose}
      footer={
        <>
          ...
          <button type="submit" form="complaint-category-form" className="auth-submit" disabled={status === 'submitting'}>
            {status === 'submitting' ? 'Saving…' : isEditMode ? 'Save changes' : 'Add category'}
          </button>
        </>
      }
    >
      <form id="complaint-category-form" className="auth-form" onSubmit={handleSubmit} noValidate>
